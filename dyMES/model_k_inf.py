...
class model:
    ...
    def pandemic_dfdI(self, n, I, params):
        # K INF:
        # (params['c0'] * params['h'] * n * np.exp(0) * (m_effective - n) - (params['d_0'] * n)
        #  m_effective = (params['m_(t-1)'] - params['c1'] * params['tau'] * params['c0'] * old_m * params['<n>_(t-1)'])

        # F(n, I) = (c0 * n * 1) * ((m_(t-1) - c1 * tau * c0 * m_(t-1) * I/G) - n) - d_0 * n

        # F1(n, I) = (c0 * n * 1)
        # F2(n, I) = ((m_(t-1) - c1 * tau * c0 * m_(t-1) * I/G) - n)

        f1 = self.params['c0'] * n * 1
